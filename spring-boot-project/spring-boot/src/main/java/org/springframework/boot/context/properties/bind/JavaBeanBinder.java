...
class JavaBeanBinder implements DataObjectBinder {
    ...
    @Override
    @Nullable
    public <T> T bind(@Nullable ConfigurationPropertyName name, Bindable<T> target, Context context, DataObjectPropertyBinder propertyBinder) {
    ...
    static class Bean<T> {
        ...
        private boolean isCandidate(Method method) {
            ...
            return !Modifier.isPrivate(modifiers) && !Modifier.isProtected(modifiers) && !Modifier.isAbstract(modifiers) && !Modifier.isStatic(modifiers) && !Object.class.equals(method.getDeclaringClass()) && !Class.class.equals(method.getDeclaringClass()) && method.getName().indexOf('$') == -1;
        }
