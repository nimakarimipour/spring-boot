...
class ValueObjectBinder implements DataObjectBinder {
    ...
    @Override
    @Nullable
    public <T> T bind(@Nullable ConfigurationPropertyName name, Bindable<T> target, Binder.Context context, DataObjectPropertyBinder propertyBinder) {
    ...
    private <T> T getNewInstanceIfPossible(Binder.Context context, ResolvableType type) {
        ...
        Assert.state(resolved == null || isEmptyDefaultValueAllowed(resolved), () -> "Parameter of type " + type + " must have a non-empty default value.");
