...
public final class DataSourceBuilder<T extends DataSource> {
    ...
    private static class DataSourceSettingsResolver {
        ...
        DataSourceSettingsResolver(@Nullable ClassLoader classLoader) {
            ...
            addIfAvailable(this.allDataSourceSettings, create(classLoader, "org.springframework.jdbc.datasource.SimpleDriverDataSource", (type) -> new DataSourceSettings(type, (aliases) -> aliases.addAliases("driver-class-name", "driver-class"))));
        ...
        private static List<DataSourceSettings> resolveAvailableDataSourceSettings(@Nullable ClassLoader classLoader) {
            ...
            addIfAvailable(providers, create(classLoader, "org.apache.tomcat.jdbc.pool.DataSource", DataSourceSettings::new));
            addIfAvailable(providers, create(classLoader, "org.apache.commons.dbcp2.BasicDataSource", DataSourceSettings::new));
        ...
        @SuppressWarnings("unchecked")
        @Nullable
        private static DataSourceSettings create(@Nullable ClassLoader classLoader, String target, Function<Class<? extends DataSource>, DataSourceSettings> factory) {
            if (ClassUtils.isPresent(target, classLoader)) {
                try {
                    Class<? extends DataSource> type = (Class<? extends DataSource>) ClassUtils.forName(target, classLoader);
