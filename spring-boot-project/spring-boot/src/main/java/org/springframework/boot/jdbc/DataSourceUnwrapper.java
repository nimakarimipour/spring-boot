...
public final class DataSourceUnwrapper {
	 ...
	 */
	 @Nullable public static <I, T extends I> T unwrap(DataSource dataSource, Class<I> unwrapInterface, Class<T> target) {
	 ...
	 * @return an object that implements the target type or {@code null}
	 */
	@Nullable public static <T> T unwrap(DataSource dataSource, Class<T> target) {
		return unwrap(dataSource, target, target);
	}

	 @Nullable private static <S> S safeUnwrap(Wrapper wrapper, Class<S> target) {
		try {
	...
	private static class DelegatingDataSourceUnwrapper {
		 ...
		 @Nullable private static DataSource getTargetDataSource(DataSource dataSource) {
