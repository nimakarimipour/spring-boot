...
class SslServerCustomizer implements JettyServerCustomizer {
    ...
    SslServerCustomizer(InetSocketAddress address, Ssl ssl, @Nullable SslStoreProvider sslStoreProvider, @Nullable Http2 http2) {
    ...
    private ServerConnector createServerConnector(Server server, SslContextFactory.Server sslContextFactory, HttpConfiguration config) {
        ...
        Assert.state(isJettyHttp2Present(), () -> "The 'org.eclipse.jetty.http2:http2-server' dependency is required for HTTP/2 support.");
    ...
    private ServerConnector createHttp11ServerConnector(Server server, HttpConfiguration config, SslContextFactory.Server sslContextFactory) {
        ...
        return new SslValidatingServerConnector(server, sslContextFactory, this.ssl.getKeyAlias(), sslConnectionFactory, connectionFactory);
    }
     ...
     */
    protected void configureSsl(SslContextFactory.Server factory, Ssl ssl, @Nullable SslStoreProvider sslStoreProvider) {
    ...
    static class SslValidatingServerConnector extends ServerConnector {
        ...
        SslValidatingServerConnector(Server server, SslContextFactory sslContextFactory, @Nullable String keyAlias, SslConnectionFactory sslConnectionFactory, HttpConnectionFactory connectionFactory) {
            super(server, sslConnectionFactory, connectionFactory);
            this.sslContextFactory = sslContextFactory;
            this.keyAlias = keyAlias;
        }

        SslValidatingServerConnector(Server server, SslContextFactory sslContextFactory, @Nullable String keyAlias, ConnectionFactory... factories) {
