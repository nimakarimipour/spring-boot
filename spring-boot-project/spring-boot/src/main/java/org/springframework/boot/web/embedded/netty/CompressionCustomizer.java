...
final class CompressionCustomizer implements NettyServerCustomizer {
    ...
    private CompressionPredicate getMimeTypesPredicate(String[] mimeTypeValues) {
        ...
        List<MimeType> mimeTypes = Arrays.stream(mimeTypeValues).map(MimeTypeUtils::parseMimeType).collect(Collectors.toList());
    ...
    private CompressionPredicate getExcludedUserAgentsPredicate(@Nullable String[] excludedUserAgents) {
        ...
        return (request, response) -> {
            ...
            return Arrays.stream(excludedUserAgents).noneMatch((candidate) -> headers.contains(HttpHeaderNames.USER_AGENT, candidate, true));
        };
    }
