...
public class SslServerCustomizer implements NettyServerCustomizer {
    ...
    protected KeyManagerFactory getKeyManagerFactory(Ssl ssl, @Nullable SslStoreProvider sslStoreProvider) {
        try {
    ...
    protected TrustManagerFactory getTrustManagerFactory(Ssl ssl, @Nullable SslStoreProvider sslStoreProvider) {
        try {
    ...
    private static final class ConfigurableAliasKeyManagerFactorySpi extends KeyManagerFactorySpi {
        ...
        protected KeyManager[] engineGetKeyManagers() {
            return Arrays.stream(this.delegate.getKeyManagers()).filter(X509ExtendedKeyManager.class::isInstance).map(X509ExtendedKeyManager.class::cast).map(this::wrap).toArray(KeyManager[]::new);
        }
